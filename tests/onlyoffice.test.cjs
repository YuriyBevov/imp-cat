...
test('ships an isolated ONLYOFFICE POC page and compose service', () => {
  ...
  assert.match(html, /id="segment-panel"/)
  assert.match(html, /id="segment-list"/)
  assert.match(client, /DocsAPI\.DocEditor/)
  assert.match(client, /\/api\/onlyoffice\/documents/)
  assert.match(client, /loadSegmentIndex/)
  assert.match(client, /icat-segment-drafts/)
  ...
  assert.match(styles, /\.segment-panel[\s\S]*width: clamp\(340px, 27vw, 430px\)/)
})
