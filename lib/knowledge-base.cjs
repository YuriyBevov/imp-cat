...
function findTextOccurrences(textValue, sourceValue, maximum = 20) {
  const text = String(textValue || '')
  const sourceText = String(sourceValue || '').trim()
  if (!sourceText || sourceText.length > text.length) return []
  const escapedSource = sourceText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const expression = new RegExp(escapedSource, 'giu')
  const offsets = []
...
function entryMatchesInText(entry, text) {
  const fullSegment = canonicalText(entry.sourceText) === canonicalText(text)
  return findTextOccurrences(text, entry.sourceText).map((range, index) => ({
...
function createMemoryKnowledgeBase(options = {}) {
  ...
  async function findMatchesInText(text, targetLanguage, searchOptions = {}) {
    ...
    return eligible.map(entry => ({ entry, score: trigramSimilarity(entry.sourceText, text) }))
...
function createPostgresKnowledgeBase(options = {}) {
  ...
  async function findMatchesInText(text, targetLanguage, searchOptions = {}) {
    ...
    const result = await pool.query(
      `SELECT * FROM icat_translation_memory
       WHERE glossary_id = $1 AND lower(target_language) = ANY($2::text[])
         ...
         AND (position(source_text in $5) > 0 OR position(lower(source_text) in lower($5)) > 0)
      ...
      [String(searchOptions.glossaryId || DEFAULT_GLOSSARY_ID), languageVariants(targetLanguage || 'ru'),
        languageVariants(sourceLanguage).includes('auto'), languageVariants(sourceLanguage), sourceText],
    )
    ...
    const fuzzy = await pool.query(
      ...
      [String(searchOptions.glossaryId || DEFAULT_GLOSSARY_ID), languageVariants(targetLanguage || 'ru'),
        languageVariants(sourceLanguage).includes('auto'), languageVariants(sourceLanguage), canonicalText(sourceText)],
    )
  ...
  async function exactRows(query, targetLanguage, limit, searchOptions) {
    ...
    const result = await pool.query(
      ...
      [String(searchOptions.glossaryId || DEFAULT_GLOSSARY_ID), languageVariants(targetLanguage || 'ru'), canonicalText(query),
