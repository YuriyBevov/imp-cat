...
function sentenceCaseText(value, language = 'auto') {
  const locale = languageLocale(language)
  const characters = Array.from(canonicalText(value).toLocaleLowerCase(locale).normalize('NFC'))
  let capitalizeNextLetter = true
  for (let index = 0; index < characters.length; index += 1) {
    const character = characters[index]
    if (/\p{L}/u.test(character)) {
      if (capitalizeNextLetter) characters[index] = character.toLocaleUpperCase(locale)
      capitalizeNextLetter = false
    }
    if (character === '.') capitalizeNextLetter = true
  }
...
function adaptTranslationCase(sourceValue, translationValue, targetLanguage = 'auto', sourceLanguage = 'auto') {
  ...
  const translation = sentenceCaseText(translationValue, targetLanguage)
  ...
  if (!startsUppercase) return translation.toLocaleLowerCase(targetLocale)
  return translation
}
...
function normalizeEntry(value = {}) {
  ...
  const rawSourceText = String(value.sourceText || '').slice(0, 100_000)
  const sourceText = sentenceCaseText(rawSourceText, sourceLanguage).slice(0, 100_000)
  const sourceCanonical = knowledgeText(rawSourceText, sourceLanguage).slice(0, 100_000)
  const translation = sentenceCaseText(String(value.translation || '').slice(0, 100_000), targetLanguage).slice(0, 100_000)
...
function publicEntry(row, score = null, matchType = null) {
  ...
  const result = {
    ...
    sourceText: sentenceCaseText(row.source_text ?? row.sourceText, sourceLanguage),
    translation: sentenceCaseText(row.translation, targetLanguage),
    sourceLanguage,
    targetLanguage,
    createdAt: row.created_at ?? row.createdAt,
    updatedAt: row.updated_at ?? row.updatedAt,
...
// A pair is stored once, but exact lookup can use either language as its input.
function orientEntries(entries, sourceLanguage, targetLanguage) {
  return entries.flatMap(entry => {
    const result = []
    if (languageMatches(sourceLanguage, entry.sourceLanguage) && languageMatches(targetLanguage, entry.targetLanguage)) result.push(entry)
    if (languageMatches(sourceLanguage, entry.targetLanguage) && languageMatches(targetLanguage, entry.sourceLanguage)) {
      result.push({ ...entry, sourceText: entry.translation, translation: entry.sourceText,
        sourceLanguage: entry.targetLanguage, targetLanguage: entry.sourceLanguage,
        sourceCanonical: knowledgeText(entry.translation, entry.targetLanguage), embedding: null })
    }
    return result
  })
}
...
function createMemoryKnowledgeBase(options = {}) {
  ...
  async function listEntries(listOptions = {}) {
    ...
    const filtered = [...entries.values()].filter(entry => (
      ...
      && (!queries.length || queries.some(query => entry.sourceCanonical.includes(query) || knowledgeText(entry.translation, entry.targetLanguage).includes(query)))
    )).sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))
    return { entries: filtered.slice(offset, offset + limit).map(entry => publicEntry(entry)), total: filtered.length, limit, offset }
  }
  ...
  async function findMatchesInText(text, targetLanguage, searchOptions = {}) {
    ...
    const eligible = orientEntries([...entries.values()].filter(entry => entry.glossaryId === glossaryId), sourceLanguage, targetLanguage)
    const exact = eligible.flatMap(entry => entryMatchesInText(entry, text))
      ...
      .slice(0, 100)
    if (exact.length || searchOptions.exactOnly) return exact
  ...
  async function search(query, targetLanguage, limit = 8, searchOptions = {}) {
    ...
    const glossaryId = String(searchOptions.glossaryId || DEFAULT_GLOSSARY_ID)
    const eligible = orientEntries([...entries.values()].filter(entry => entry.glossaryId === glossaryId), sourceLanguage, targetLanguage)
    const exact = eligible.filter(entry => knowledgeText(entry.sourceText, entry.sourceLanguage) === knowledgeText(query, entry.sourceLanguage))
      .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))
  ...
  async function addMany(values, requestOptions = {}) {
    ...
    for (const candidate of candidates) {
      ...
      if (existing) {
      const same = knowledgeText(existing.translation, candidate.targetLanguage) === knowledgeText(candidate.translation, candidate.targetLanguage)
...
function createPostgresKnowledgeBase(options = {}) {
  ...
  async function listEntries(listOptions = {}) {
    ...
    return { entries: result.rows.map(row => publicEntry(row)), total: countResult.rows[0]?.total || 0, limit, offset }
  }
  ...
  async function findMatchesInText(text, targetLanguage, searchOptions = {}) {
    ...
    const exact = (await exactCandidates(sourceLanguage, targetLanguage, searchOptions))
  ...
  async function exactCandidates(sourceLanguage, targetLanguage, searchOptions) {
    const cacheKey = JSON.stringify([searchOptions.glossaryId || DEFAULT_GLOSSARY_ID, sourceLanguage, targetLanguage])
    if (searchOptions.candidateCache?.has(cacheKey)) return searchOptions.candidateCache.get(cacheKey)
    // Do not use database lower(): its locale can differ from the entry language
    // (notably Turkish I/İ). Word boundaries and offsets are checked in JS.
    const result = await pool.query(
      `SELECT * FROM icat_translation_memory
       WHERE glossary_id = $1 AND (
         (($2::boolean OR lower(source_language) = ANY($3::text[]) OR lower(source_language) = 'auto')
          AND ($4::boolean OR lower(target_language) = ANY($5::text[]) OR lower(target_language) = 'auto'))
         OR (($2::boolean OR lower(target_language) = ANY($3::text[]) OR lower(target_language) = 'auto')
          AND ($4::boolean OR lower(source_language) = ANY($5::text[]) OR lower(source_language) = 'auto'))
       ) ORDER BY updated_at DESC`,
      [String(searchOptions.glossaryId || DEFAULT_GLOSSARY_ID), languageVariants(sourceLanguage).includes('auto'),
        languageVariants(sourceLanguage), languageVariants(targetLanguage).includes('auto'), languageVariants(targetLanguage)],
    )
    const candidates = orientEntries(result.rows.map(row => publicEntry(row)), sourceLanguage, targetLanguage)
    searchOptions.candidateCache?.set(cacheKey, candidates)
    return candidates
  }

  async function exactRows(query, targetLanguage, limit, searchOptions) {
    const entries = await exactCandidates(searchOptions.sourceLanguage || 'auto', targetLanguage, searchOptions)
    return entries.filter(entry => knowledgeText(query, entry.sourceLanguage) === knowledgeText(entry.sourceText, entry.sourceLanguage)).slice(0, limit)
  }
