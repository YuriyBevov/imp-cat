...
function pageGridSize(page) {
  const availableWidth = Math.max(1, Number(page.gridAvailableBounds?.width) || Number(page.contentBounds?.width) || 1)
  const approximateOuterSize = availableWidth / 16 * 1.5
  const outerSize = Math.max(4, Math.round(approximateOuterSize / 4) * 4)
  return outerSize / 4
}
