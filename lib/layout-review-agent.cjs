...
function buildLayoutReviewPrompt(page, objects) {
  ...
  return [
    ...
    'Не меняйте и не переводите текст. Предлагайте только координаты x/y, ширину/высоту, размер шрифта и выравнивание. Ширина и высота будут сохранены только как рекомендации: автоматически их менять нельзя.',
...
function applyLayoutReview(scene, reviews, threshold = 0.72) {
  ...
  for (const review of reviews || []) {
    for (const adjustment of review.adjustments || []) {
      ...
      if (adjustment.width != null || adjustment.height != null) {
        recommendations.push({
          ...
          fontSizePx: null,
          textAlign: null,
          reason: `Размер сегмента изменяется только пользователем. ${adjustment.reason}`.trim(),
        })
      }
      const hasAutomaticChange = adjustment.x != null || adjustment.y != null || adjustment.fontSizePx != null || adjustment.textAlign
      ...
      object.width = Math.min(area.width, Math.max(12, object.width))
      object.height = Math.min(area.height, Math.max(12, object.height))
      if (adjustment.x != null) object.x = Math.max(area.x, Math.min(area.x + area.width - object.width, adjustment.x))
      if (adjustment.y != null) object.y = Math.max(area.y, Math.min(area.y + area.height - object.height, adjustment.y))
