...
function buildDocumentPrompt(filename, manifest) {
  ...
  return [
    ...
    'Для каждой распознанной таблицы обязательно создавай отдельный table_cell для каждой визуальной ячейки. Всем её ячейкам задай одинаковый tableId, нулевые rowIndex и columnIndex, а также rowSpan и columnSpan (обычно 1). Тип table используй только когда границы строк и колонок невозможно определить.',
