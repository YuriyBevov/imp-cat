...
function normalizeStyle(style) {
  const families = ['Arial', 'Aptos', 'Calibri', 'Cambria', 'Times New Roman', 'Georgia', 'Verdana', 'Tahoma', 'Courier New']
  return {
    fontFamily: families.find(name => name.toLowerCase() === String(style?.fontFamily || '').toLowerCase()) || 'Arial',
    fontSizePt: finite(style?.fontSizePt, 10.5, 4, 96),
    fontWeight: finite(style?.fontWeight, 400, 100, 900) >= 600 ? 700 : 400,
    fontStyle: style?.fontStyle === 'italic' ? 'italic' : 'normal',
    textAlign: ['left', 'center', 'right', 'justify'].includes(style?.textAlign) ? style.textAlign : 'left',
    lineHeight: finite(style?.lineHeight, 1.2, .8, 3),
    color: /^#[0-9a-f]{6}$/i.test(String(style?.color || '')) ? style.color.toLowerCase() : '#000000',
  }
}
...
function normalizeCodexAnalysis(raw, manifest) {
  if (!raw || !Array.isArray(raw.pages)) throw new Error('Агент не вернул массив страниц')
  ...
  if (raw.pages.length !== manifestPages.length) {
    throw new Error(`Агент вернул ${raw.pages.length} стр. вместо ${manifestPages.length}`)
  }
  ...
  const pages = manifestPages.map((manifestPage, pageIndex) => {
    ...
    if (!rawPage) throw new Error(`Агент пропустил страницу ${pageIndex + 1}`)
    ...
    const segments = rawPage.segments.map((segment, index) => {
      segmentCount += 1
      if (segmentCount > SEGMENT_LIMIT) throw new Error(`Агент вернул больше ${SEGMENT_LIMIT} сегментов`)
      ...
      return {
        ...
        tableId: type === 'table_cell' ? String(segment?.tableId || segment?.flowGroup || `page-${pageIndex + 1}-table`).slice(0, 120) : null,
        rowIndex: type === 'table_cell' && segment?.rowIndex != null && Number.isInteger(Number(segment.rowIndex)) ? Math.max(0, Math.trunc(Number(segment.rowIndex))) : null,
        columnIndex: type === 'table_cell' && segment?.columnIndex != null && Number.isInteger(Number(segment.columnIndex)) ? Math.max(0, Math.trunc(Number(segment.columnIndex))) : null,
        rowSpan: type === 'table_cell' ? Math.max(1, Math.trunc(Number(segment?.rowSpan) || 1)) : null,
        columnSpan: type === 'table_cell' ? Math.max(1, Math.trunc(Number(segment?.columnSpan) || 1)) : null,
        regions,
...
function buildDocumentPrompt(filename, manifest) {
  const pageDescription = manifest.pages.map(page => (
    `pageIndex=${page.index}: страница документа ${(page.documentPageIndex ?? page.index) + 1}, ${page.image}, ${page.width}×${page.height} px`
  )).join('; ')
  const batchDescription = manifest.batchCount > 1
    ? `Это пакет ${manifest.batchNumber} из ${manifest.batchCount}; всего в документе ${manifest.documentPageCount} страниц.`
    : ''
  return [
    ...
    'В JSON поле pageIndex используй номер из описания выше: первое изображение в этом запросе имеет pageIndex=0.',
    ...
    'Для каждой распознанной таблицы обязательно создавай отдельный table_cell для каждой визуальной ячейки. Никогда не возвращай всю строку таблицы одним сегментом и не объединяй её колонки через табуляцию, пробелы или переносы. В sourceText одного table_cell должен находиться текст только одной физической ячейки. Всем ячейкам одной таблицы задай одинаковый tableId; rowIndex и columnIndex начинаются с 0 и обязаны соответствовать фактической строке и колонке, поэтому соседние ячейки строки имеют разные columnIndex. rowSpan и columnSpan обычно равны 1. Тип table используй только когда границы строк и колонок действительно невозможно определить.',
    'Обнаруживай stamp, seal, signature, handwriting, logo и image. Для stamp/seal/handwriting запиши в sourceText весь уверенно читаемый текст; для signature не транскрибируй графический росчерк, но сохрани читаемую подпись или подпись-метку, если она напечатана. Если текст не читается, оставь sourceText пустым и объясни это в notes. Не угадывай символы.',
    ...
    'Для типографики оцени размер шрифта, обычное/жирное начертание (fontWeight строго 400 или 700), курсив, выравнивание и относительную высоту строки lineHeight. Подбери визуально близкое семейство из Arial, Aptos, Calibri, Cambria, Times New Roman, Georgia, Verdana, Tahoma, Courier New; при неуверенности используй Arial. Цвет текста передай как #RRGGBB, без учёта фона и артефактов скана. Сохраняй отдельные колонки, строки таблиц и порядок чтения; flowGroup объединяет только логически связанный поток одной колонки. Не приписывай текст, которого нет на изображении.',
...
async function analyzeDocumentWithCodex(options) {
  ...
  let result
  try {
    result = await runProcess(codexBin, args, timeoutMs, { signal })
  } catch (error) {
