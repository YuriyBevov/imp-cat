...
function createStudioRouter(options) {
  ...
  let activeTranslationProvider = String(process.env.TRANSLATION_PROVIDER || 'aitunnel').toLowerCase() === 'codex'
    ? 'codex'
    : 'aitunnel'
  const provider = {
  ...
  const requestedAnalysisMode = String(process.env.DOCUMENT_ANALYSIS_MODE || 'codex').toLowerCase()
  const documentAgent = {
    mode: ['codex', 'local'].includes(requestedAnalysisMode) ? requestedAnalysisMode : 'codex',
  ...
  router.get('/status', async (request, response) => {
    ...
    response.json({
      ...
      documentAnalysisMode: documentAgent.mode,
      documentAgent: documentAgent.mode === 'codex' ? 'Codex CLI' : 'Legacy local OCR',
      documentAgentModel: documentAgent.mode === 'codex' ? documentAgent.model : null,
      documentAgentUsesChatGptAuth: documentAgent.mode === 'codex',
      ...
      translationProvider: activeTranslationProvider,
      translationProviderConfigured: activeTranslationProvider === 'codex' ? settings.codexConfigured : settings.aitunnelConfigured,
      translationModel: activeTranslationProvider === 'codex' ? documentAgent.model : provider.model || null,
  ...
  router.put('/provider', async (request, response, next) => {
    try {
      const requestedProvider = String(request.body?.provider || activeTranslationProvider).toLowerCase()
      ...
      if (request.body?.encryptedApiKey) credentialVault.setEncrypted(request.body.encryptedApiKey)
      if (model) provider.model = model
      activeTranslationProvider = requestedProvider
  ...
  router.delete('/provider/key', async (request, response) => {
    credentialVault.clearSession()
    response.set('Cache-Control', 'no-store').json(providerSettings(await readCodexStatus()))
  })

  async function storeUpload(buffer, rawFilename) {
    if (!Buffer.isBuffer(buffer) || buffer.length < 16) throw httpError(400, 'Файл пуст или повреждён')
  ...
  async function processStoredUpload(upload, reportProgress = () => {}) {
    ...
    if (documentAgent.mode === 'codex') analyzerArguments.push('--no-ocr')
    ...
    const analysis = documentAgent.mode === 'codex'
      ? await analyzeDocumentWithCodex({
        codexBin: documentAgent.codexBin,
        model: documentAgent.model,
        workdir: directory,
        documentDirectory: directory,
        filename,
        manifest: renderedAnalysis,
    ...
    if (documentAgent.mode === 'local' && provider.apiKey && provider.visionModel && provider.visionEnabled && provider.visionAutoAugment) {
      reportProgress({ stage: 'vision-review', progress: 72, message: 'Дополнительно проверяем распознавание' })
      try {
    ...
    const sceneOptions = {
      ...
      fitRasterToA4: false,
      model: documentAgent.mode === 'codex' ? documentAgent.model : null,
    }
    const scene = documentAgent.mode === 'codex'
      ? buildSceneFromAgent(analysis, sceneOptions)
      : buildScene(analysis, sceneOptions)
    ...
    const metadata = {
      ...
      analysisMode: documentAgent.mode,
      analysisModel: documentAgent.mode === 'codex' ? documentAgent.model : null,
      visionAugmentation,
      visionError,
    }
  ...
  router.post(
    ...
    async (request, response, next) => {
      try {
        const upload = await storeUpload(request.body, request.get('X-File-Name'))
        const documentData = await processStoredUpload(upload)
    ...
    async (request, response, next) => {
      try {
        ...
        const job = jobs.enqueue({
          ...
          task: async update => {
            const documentData = await processStoredUpload(upload, update)
            return {
  ...
  router.get('/documents', async (request, response, next) => {
    try {
      ...
      for (const entry of entries) {
        ...
        try { documents.push(await readMetadata(entry.name)) } catch (error) {
  ...
  router.post('/documents/:id/agent/reanalyze', async (request, response, next) => {
    try {
      if (documentAgent.mode !== 'codex') throw httpError(409, 'Повторный агентный анализ доступен только в режиме Codex')
      const metadata = await readMetadata(request.params.id)
      ...
      const analysis = await analyzeDocumentWithCodex({
        codexBin: documentAgent.codexBin,
        model: documentAgent.model,
        workdir: directory,
        documentDirectory: directory,
        ...
        manifest,
        schemaPath: documentAgentSchema,
        outputPath: path.join(directory, `codex-analysis-r${metadata.revision + 1}.json`),
        timeoutMs: documentAgent.timeoutMs,
        runProcess,
      })
      ...
      const updatedMetadata = {
        ...
        analyzer: analysis.engine,
        analysisMode: 'codex',
        analysisModel: documentAgent.model,
      }
      await Promise.all([writeJson(scenePath(metadata.id), scene), writeJson(metadataPath(metadata.id), updatedMetadata)])
      response.json(serializeDocument(updatedMetadata, scene))
    } catch (error) { next(error) }
  ...
  router.post('/documents/:id/translate', async (request, response, next) => {
    try {
      ...
      const codexTranslationStatus = activeTranslationProvider === 'codex' ? await readCodexStatus() : null
      const machineTranslationReady = activeTranslationProvider === 'codex'
      ...
      if (pending.length && machineTranslationReady) {
        for (const batch of createTranslationBatches(pending)) {
          const result = activeTranslationProvider === 'codex'
          ...
          for (const object of batch) {
            ...
            translated.push({ id: object.id, source: activeTranslationProvider })
          }
        }
      }
      scene.updatedAt = new Date().toISOString()
