...
const { buildScene, findMemoryMatches, validateScene } = require('./studio-model.cjs')
...
function normalizeScene(scene, documentId, title) {
  ...
  const objects = scene.objects.map((object, index) => {
    ...
    return {
      ...
      sourceText: String(object.sourceText || '').slice(0, 100_000),
      translation: String(object.translation || '').slice(0, 100_000),
...
function createStudioRouter(options) {
  ...
  const analyzerSource = path.join(rootDir, 'scripts', 'macos_document_analyzer.swift')
  const analyzerBinary = path.join(rootDir, '.build', 'studio-analyzer')
  ...
  const provider = {
    apiUrl: process.env.TRANSLATION_API_URL || process.env.AI_API_URL || 'https://api.openai.com/v1/chat/completions',
  ...
  router.get('/status', (request, response) => {
    response.json({
      ...
      localAnalyzerAvailable: process.platform === 'darwin',
  ...
  router.post(
    ...
    async (request, response, next) => {
      try {
        ...
        if (process.platform !== 'darwin') throw httpError(501, 'В этой тестовой сборке локальный OCR настроен для macOS')
        ...
        const result = await runProcess(analyzerBinary, [sourcePath, pagesDirectory, analysisPath], 180_000)
  ...
  async function ensureAnalyzer() {
    const [sourceStat, binaryStat] = await Promise.all([
      fs.promises.stat(analyzerSource),
      fs.promises.stat(analyzerBinary).catch(() => null),
    ])
    if (binaryStat && binaryStat.mtimeMs >= sourceStat.mtimeMs) return
    await fs.promises.mkdir(path.dirname(analyzerBinary), { recursive: true })
    const result = await runProcess('swiftc', [analyzerSource, '-o', analyzerBinary], 120_000)
    if (result.code !== 0) throw httpError(500, result.stderr.trim() || 'Не удалось подготовить локальный OCR')
  }
