...
function normalizeScene(scene, documentId, title) {
  ...
  const pages = scene.pages.map((page, index) => {
    ...
    const sourceAspectHeight = Math.max(300, Number((widthPx * sourceHeight / sourceWidth).toFixed(2)))
  ...
  return {
    ...
    layoutInitializationVersion: scene.layoutInitializationVersion == null
      ? (workflowStage >= 3 ? 1 : 0)
      : Math.max(0, Math.min(2, Math.trunc(Number(scene.layoutInitializationVersion) || 0))),
...
function createStudioRouter(options) {
  ...
  router.get('/documents/:id', async (request, response, next) => {
    try {
      ...
      const needsMigration = Number(metadata.sceneNormalizationVersion || 0) < 2
