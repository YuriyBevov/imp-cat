...
function normalizeInstructionPreset(value) {
  ...
  const fallbackTitle = instruction.split(/\r?\n/, 1)[0].slice(0, 80)
  return {
    id: String(value?.id || '').slice(0, 64),
    title: String(value?.title || fallbackTitle || 'Готовая инструкция').trim().slice(0, 120) || 'Готовая инструкция',
    instruction,
    ...
    try {
      ...
      const result = await updateInstructionPresets(async presets => {
        ...
        const candidate = normalizeInstructionPreset({
          ...current,
          ...(Object.hasOwn(request.body || {}, 'title') ? { title: request.body.title } : {}),
