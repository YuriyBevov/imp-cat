...
function normalizeScene(scene, documentId, title) {
  ...
  const pages = scene.pages.map((page, index) => {
    ...
    const heightPx = Math.max(minimumA4PageHeight(widthPx), finite(page.heightPx, 1123, 300, 4_000))
...
function createStudioRouter(options) {
  ...
  router.get('/documents/:id', async (request, response, next) => {
    try {
      ...
      try {
        ;({ changed: synchronized } = await refreshKnowledgeBaseMatches(scene))
      } catch {
        try { synchronized = await reconcileKnowledgeBaseEntryReferences(scene) } catch {}
      }
      if (synchronized) {
        const updatedAt = new Date().toISOString()
        scene.updatedAt = updatedAt
        metadata = { ...metadata, updatedAt, revision: Number(metadata.revision || 0) + 1 }
