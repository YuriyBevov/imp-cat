        ...
        for (const unit of units) {
          ...
          if (match) {
            const formattedMatch = {
              ...match,
              translation: formatServiceTranslation(object.type, match.translation),
            }
            unit.memorySuggestion = {
              entryId: formattedMatch.id,
              translation: formattedMatch.translation,
              score: 1,
              matchType: 'exact',
              targetLanguage: scene.targetLanguage,
            }
            unit.status = 'memory-suggested'
            suggested.push({ objectId: object.id, unitId: unit.id, match: formattedMatch })
          } else {
