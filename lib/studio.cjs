...
function createStudioRouter(options) {
  ...
  router.get('/status', async (request, response) => {
    ...
    let codexStatusError = null
    if (documentAgent.mode === 'codex') {
      try {
        const result = await runProcess(documentAgent.codexBin, ['login', 'status'], 10_000)
        codexAvailable = result.code === 0
        codexAuthenticated = result.code === 0 && /logged in/iu.test(`${result.stdout}\n${result.stderr}`)
        if (result.code !== 0) codexStatusError = String(result.stderr || result.stdout || 'Codex login не настроен').trim().slice(0, 300)
      } catch (error) {
        codexStatusError = error?.code === 'ENOENT' ? 'Codex CLI не найден' : String(error?.message || error).slice(0, 300)
      }
    }
    response.json({
