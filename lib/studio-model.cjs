...
const TRANSLATABLE_TYPES = new Set(['text', 'table', 'table_cell'])
...
function buildSceneFromAgent(analysis, options = {}) {
  ...
  for (const page of analysis.pages || []) {
    ...
    const pageObjects = (page.segments || []).map((segment, index) => {
      ...
      const lineHeight = clamp(segment.style?.lineHeight || 1.2, 0.8, 3)
      ...
      return {
        ...
        style: {
          fontFamily: String(segment.style?.fontFamily || 'Arial'),
          fontSizePx: Number(fontSizePx.toFixed(2)),
          fontWeight: clamp(segment.style?.fontWeight || 400, 100, 900),
          ...
          textAlign: ['left', 'center', 'right', 'justify'].includes(segment.style?.textAlign)
            ...
            : 'left',
          lineHeight,
          color: /^#[0-9a-f]{6}$/i.test(segment.style?.color) ? segment.style.color : '#111827',
        },
        sourceLineIds: [],
