  ...
  return (page.lines || [])
    ...
    .map((line, index) => ({
      ...
      alternatives: Array.isArray(line.alternatives)
        ...
        : [],
      source: String(line.source || 'local-ocr'),
  ...
  for (const page of analysis.pages || []) {
    ...
    pages.push({
      ...
      ocrStats: {
        primaryLines: 0,
        secondaryLines: 0,
        mergedLines: pageObjects.length,
        nativeLines: 0,
        tiledLines: 0,
        agentAddedLines: pageObjects.length,
        agentCorrectedLines: 0,
        agentRemovedLines: 0,
      },
    })
    objects.push(...pageObjects)
...
function buildOcrReview(scene, providerSuggestions = []) {
  ...
  for (const object of scene.objects || []) {
    if (object.excluded || !TRANSLATABLE_TYPES.has(object.type)) continue
